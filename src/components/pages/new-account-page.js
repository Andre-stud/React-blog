...
function NewAccountPage() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmitForm)} className="sing-page">
      ...
      {nameEmailError ? <span style={{ color: '#F5222D' }}>Name or email already taken!</span> : null}
      ...
      {errors?.repeatPassword && (
        <span style={{ color: '#F5222D' }}>{errors?.repeatPassword?.message || 'Passwords must match.'}</span>
      )}
      ...
      {errors?.checkbox && (
        <span style={{ color: '#F5222D' }}>
          {errors?.checkbox?.message || 'Give consent to the processing of personal data.'}
        </span>
      )}
